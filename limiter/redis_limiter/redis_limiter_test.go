...
func TestRedisLimiter_RedisWaitAllowN(t *testing.T) {
	...
	globalwg.Add(2)
	go RrunTest("test1", &globalwg)
	go RrunTest("test2", &globalwg)
	globalwg.Wait()
}
	...
	for i := 0; i < 10; i++ {
		for _, chatBot := range chatBots {
			...
			go func(chatBot string, i int) {
				...
				//var err error
				startTime := time.Now()
				withTimeout, cancelFunc := context.WithTimeout(context.Background(), 100*time.Second)
				defer cancelFunc()
				ok, err := limiters.RedisWaitAllowN(withTimeout, chatBot, redis_rate.PerSecond(6), 2)
				if err != nil {
